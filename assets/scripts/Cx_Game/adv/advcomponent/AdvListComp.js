...
cc.Class({
    ...
    start () {
        cx_TimeMgr.delayFunc(0.03,()=>{
            this._updateList();
            this._listUpdateOver = true;
        },this)
        
    },
    onEnable () {
        if(this._listUpdateOver = true) {
            this._imgInfoList = [];
            //cx_DyData.clickedDcAdvArr = [];//已经跳转过的游戏记录
    ...
    _creatNode () {
        ...
        lbl.lineHeight = this.fontSize + 5;
    ...
    _getGoInfoList:function(tag,appid) {
        let arr = cx_DyData.clickedDcAdvArr;
        /*let is = true;
    ...
    _replaceBtnNode (target) {
        ...
        if(datalist.length > maxShowCells)
        {
            ...
            if(tmparr.length > 0) {
                let st = cx_jsTools.getRandomNum(0,tmparr.length - 1);
    ...
    _updateList(){
        ...
        if(this.listSize.width === 0) {
            node.size = cc.size(this.node.width,this.node.height);
        } else {
