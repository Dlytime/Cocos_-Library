...
cc.Class({
    ...
    properties: {
        lblSf_lose:cc.SpriteFrame,
        lblSf_win:cc.SpriteFrame,
        reviveSt:{
            default:0,
    ...
    onLoad () {
        ...
        this.threeBtn = frame.getChildByName("threeBtn");
        this.fuhuoBtn = frame.getChildByName("fuhuoBtn");
    ...
    init:function(mManager) {
        ...
        this.setUI();
        this.setOther();
        cx_DyDataMgr.setRoundInfo("isthreeAward",false);
    ...
    setBtn() {
        if(cx_DyDataMgr.getResult() == 'win') {
            this.fuhuoBtn.active = false;
            this.threeBtn.active = true;
            if(this.lblSf_win) {
                this.actNode.getChildByName("goSpr").getComponent(cc.Sprite).spriteFrame = this.lblSf_win;
            }
        }
        else {
            this.threeBtn.active = false;
            if(this.reviveSt > 0) {
                this.fuhuoBtn.active = true;
            } else {
                this.fuhuoBtn.active = false;
            }
            if(this.lblSf_lose) {
                this.actNode.getChildByName("goSpr").getComponent(cc.Sprite).spriteFrame = this.lblSf_lose;
            }
        }
    },
    setContent:function() {
    ...
    setBackTime:function() {
        ...
        let loseSpr = this._loseContent.getChildByName("loseSpr");
        if(this.reviveSt <= 0) {
            timeBg.active = false;
            loseSpr.active = true;
            timelbl.string = "";
    ...
    getGold:function() {
        if(cx_DyDataMgr.getResult() == "lose") {
            this.close({type:"go"});
            return;
        }
